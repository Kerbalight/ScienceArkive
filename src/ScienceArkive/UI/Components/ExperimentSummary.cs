    ...
    public class ExperimentSummary
    {
        ...
        public void BindExperiment(ExperimentDefinition experiment, CelestialBodyComponent celestialBody, List<CompletedResearchReport> reports)
        {
            ...
            foreach (ScienceSitutation situation in Enum.GetValues(typeof(ScienceSitutation)))
            {
                ...
                situationLabel.Q<Label>("situation-label").text = "// <color=#E7CA76>" + situation.GetTranslatedDescription().ToUpper() + "</color>";
                ...
                if (regionRequired)
                {
                    ...
                    foreach (var region in regions)
                    {
                        ...
                        var isRegionFlavorPresent = experiment.DataFlavorDescriptions.Any(flavor => flavor.ResearchLocationID == regionResearchLocation.ResearchLocationId);
                        if (!isRegionFlavorPresent) continue;
                        
                        var regionReports = reports.Where(r => r.ResearchLocationID == regionResearchLocation.ResearchLocationId && r.ExperimentID == expId);
